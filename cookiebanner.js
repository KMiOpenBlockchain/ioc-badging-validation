function runAfterHTMLIncluded() {
    ...
    if (!localStorage.getItem('openBlockchainIoCBadgingValidationcookieConsent')) {
        cookieBanner.style.display = 'block';
    }

    // Function to check if the user has given consent
    function hasConsent() {
        const consent = localStorage.getItem('openBlockchainIoCBadgingValidationcookieConsent');
        return consent !== null && consent === 'true';
    }

    noCookies.addEventListener('click', function () {
        localStorage.setItem('openBlockchainIoCBadgingValidationcookieConsent', 'true');
